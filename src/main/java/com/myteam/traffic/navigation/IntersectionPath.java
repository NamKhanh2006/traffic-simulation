...
public final class IntersectionPath {
    ...
    // Tọa độ thực tế cho đường cong Bezier
    ...
    // --- Biến hỗ trợ tương thích ngược (Backward Compatibility) ---
    // Giữ lại để các file cũ (như RoadContext) gọi không bị lỗi đỏ
    ...
    public IntersectionPath(Intersection intersection, ConnectionPoint entry, ConnectionPoint exit,
        ...
        // --- TOÁN HỌC BEZIER CURVE CHO ĐỘ CONG MƯỢT MÀ ---
        double dist = Math.hypot(endX - startX, endY - startY);
        double pull = dist * 0.45; // Hệ số uốn cong
        ...
        // Tính góc quét cơ bản để các hàm cũ không bị lỗi
    ...
    /** Nội suy tọa độ và góc xoay dựa trên đường cong Bezier */
    public double[] sampleAt(double s) {
        double t = Math.max(0.0, Math.min(1.0, s / pathLength));
        double u = 1.0 - t;

        // Công thức Cubic Bezier tính Tọa độ X, Y
        ...
        // Công thức Đạo hàm (Tangent) để tính Góc xoay của đầu xe
