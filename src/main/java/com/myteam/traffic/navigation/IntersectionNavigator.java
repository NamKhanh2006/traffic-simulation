...
public class IntersectionNavigator {
    ...
    public boolean canMerge(Vehicle entering, Intersection intersection, List<Vehicle> allVehicles) {
        ...
        // Điểm đầu xe chuẩn bị tiến vào giao lộ
        ...
        for (Vehicle other : allVehicles) {
            ...
            // Chỉ xét những xe đã nằm bên TRONG giao lộ
            if (other.getTravelMode() == TravelMode.ON_INTERSECTION_PATH &&
                    other.getCurrentIntersection() == intersection) {

                // Nếu có xe đang trong giao lộ, và khoảng cách từ nó đến điểm vào của xe này < 50m
                // -> Không được phép nhập làn, phải đứng ngoài chờ!
                double distToEntry = other.getPosition().distanceTo(new com.myteam.traffic.model.geometry.Position(entryX, entryY));
                if (distToEntry < 50.0) {
                    return false;
                }
            }
