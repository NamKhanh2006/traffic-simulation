...
public class VehicleRenderer {
    ...
    public void render(GraphicsContext gc, Vehicle vehicle, boolean isGraphicMode) {
        ...
        double angle = vehicle.getAngle(); // Lấy hướng (góc) của xe
        ...
        if (!isGraphicMode) {
            ...
            gc.fillText(vehicle.getType(), -width / 2 + 5, -height / 2 + 15); // Ghi loại xe lên khối
        } else {
            // 2. Chế độ ĐỒ HỌA: Vẽ hình ảnh Image
            Image imageToDraw = carImage; // Mặc định là ô tô
            
            String type = vehicle.getType() != null ? vehicle.getType() : "";
            switch (type) {
                case "Ambulance":
