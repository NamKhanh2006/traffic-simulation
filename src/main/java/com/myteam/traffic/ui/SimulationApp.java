...
public class SimulationApp extends Application {
    ...
    public void start(Stage stage) {
        ...
        for (ToggleButton b : new ToggleButton[] { btnModePan, btnModeDraw, btnModeEdit, btnModeDelLight, btnModeSim })
            b.setToggleGroup(modeGroup);
        btnModePan.setSelected(true);

        HBox modeBox = new HBox(4, btnModePan, btnModeDraw, btnModeEdit, btnModeDelLight, btnModeSim);
        modeBox.setAlignment(Pos.CENTER_LEFT);

        Button btnUndo = actionBtn("↩ Hoàn tác", "Ctrl + Z");
        Button btnClear = actionBtn("🗑 Clear", "Xóa toàn bộ");
        ...
        HBox topBar = new HBox(10,
                modeBox,
                vsep(), new HBox(4, btnUndo, btnClear, btnDemoNet),
        ...
        StackPane ctxStack = new StackPane(ctxPan, ctxDraw, ctxEdit, ctxDelLight, ctxSim);
        ctxPan.setVisible(true);
        for (HBox h : new HBox[] { ctxDraw, ctxEdit, ctxDelLight, ctxSim }) {
            h.setVisible(false);
        ...
        modeGroup.selectedToggleProperty().addListener((obs, old, nw) -> {
            ...
            if (nw == btnModeDelLight)
                view.setInteractionType(SimulationView.InteractionType.DELETE_LIGHT);
            if (nw == btnModeSim)
                view.setInteractionType(SimulationView.InteractionType.PAN);
        });
    ...
    private HBox buildCtxSimulation(SimulationView view, com.myteam.traffic.controller.VehicleSpawner spawner) {
        ...
        ToggleButton btnDelMode = smallToggle("🗑 Xóa 1 xe", false);
        btnDelMode.setOnAction(e -> {
            view.setDeleteVehicleMode(btnDelMode.isSelected());
        });

        Button btnClearAll = actionBtn("🗑 Clear xe", "Xóa toàn bộ phương tiện đang chạy");
