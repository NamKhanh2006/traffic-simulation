...
public class SimulationApp extends Application {
    ...
    private Set<Lane.VehicleCategory> allowedVehicles = EnumSet.of(Lane.VehicleCategory.CAR,
            Lane.VehicleCategory.MOTORBIKE,
            Lane.VehicleCategory.BUS);
    private Set<Lane.Movement> currentMovements = EnumSet.of(Lane.Movement.STRAIGHT, Lane.Movement.LEFT,
            Lane.Movement.RIGHT);
    ...
    private Lane bwdLane(int idx) {
        ...
        if (currentMovements.contains(Lane.Movement.STRAIGHT))
            bwdMov.add(Lane.Movement.STRAIGHT);
        if (currentMovements.contains(Lane.Movement.LEFT))
            bwdMov.add(Lane.Movement.RIGHT);
        if (currentMovements.contains(Lane.Movement.RIGHT))
            bwdMov.add(Lane.Movement.LEFT);
        if (currentMovements.contains(Lane.Movement.U_TURN))
            bwdMov.add(Lane.Movement.U_TURN);
        if (bwdMov.isEmpty())
            bwdMov.add(Lane.Movement.STRAIGHT);
    ...
    /**
     * Static fallback — dùng khi laneFactory chưa được set (chiều rộng 3.5m mặc
     * định).
     */
    public static List<Lane> createLanes(int n) {
        ...
        for (int i = 0; i < fwd; i++)
            l.add(new Lane(i, Lane.Direction.FORWARD, 3.5,
        ...
        for (int i = 0; i < bwd; i++)
            l.add(new Lane(fwd + i, Lane.Direction.BACKWARD, 3.5,
    ...
    public void start(Stage stage) {
        ...
        com.myteam.traffic.controller.TrafficController trafficController = new com.myteam.traffic.controller.TrafficController(
                network);

        // Đăng ký các luật giao thông toàn cục
        trafficController.addRule(new com.myteam.traffic.rule.SignalRule());
        trafficController.addRule(new com.myteam.traffic.rule.MarkingRule());
        trafficController.addRule(new com.myteam.traffic.rule.DistanceRule(10.0, null));

        com.myteam.traffic.controller.VehicleSpawner spawner = new com.myteam.traffic.controller.VehicleSpawner(
                trafficController, network);
        ...
        btnClear.setOnAction(e -> {
            view.saveSnapshot();
            view.setNetwork(new RoadNetwork());
        });

        CheckBox cbGrid = styledCb("Lưới", true, e -> view.setShowGrid(((CheckBox) e.getSource()).isSelected()));
        CheckBox cbLabels = styledCb("Nhãn đường", true,
                e -> view.setShowLabels(((CheckBox) e.getSource()).isSelected()));
        ...
        modeGroup.selectedToggleProperty().addListener((obs, old, nw) -> {
            ...
            if (nw == btnModePan)
                view.setInteractionType(SimulationView.InteractionType.PAN);
            if (nw == btnModeDraw)
                view.setInteractionType(SimulationView.InteractionType.DRAW_ROAD);
            if (nw == btnModeEdit)
                view.setInteractionType(SimulationView.InteractionType.EDIT_MARKINGS);
            if (nw == btnModeInter)
                view.setInteractionType(SimulationView.InteractionType.PLACE_INTERSECTION);
            if (nw == btnModeDelete)
                view.setInteractionType(SimulationView.InteractionType.DELETE);
            if (nw == btnModeSim)
                view.setInteractionType(SimulationView.InteractionType.PAN);
            if (nw == btnModeLight)
                view.setInteractionType(SimulationView.InteractionType.PLACE_TRAFFIC_LIGHT);
        });
    ...
    private HBox buildCtxDraw(SimulationView view) {
        ...
        spinLane.setStyle(
                "-fx-background-color:#2a3550; -fx-background-radius:4; -fx-border-color:#3a4560; -fx-border-radius:4;");
        spinLane.getEditor().setStyle(
                "-fx-background-color:#2a3550; -fx-text-fill:#c8d0e8; -fx-font-weight:bold; -fx-font-size:12px; -fx-alignment:center;");
        ...
        spinLane.getEditor().setOnAction(e -> {
            try {
                ...
                spinLane.getEditor().setText(spinLane.getValue().toString());
            }
        ...
        spinLane.getEditor().focusedProperty().addListener((ob, ov, focused) -> {
            if (!focused)
                spinLane.getEditor().getOnAction().handle(null);
        });
        spinLane.valueProperty().addListener((ob, ov, nv) -> {
            if (nv != null)
                view.setCurrentLaneConfig(nv);
        });
        ...
        tfW.setPrefWidth(46);
        tfW.setStyle(
                "-fx-background-color:#2a3550; -fx-text-fill:#e8d44d; -fx-font-size:12px; -fx-border-color:#3a4560;");
        ...
        tfW.focusedProperty().addListener((ob, ov, focused) -> {
            if (!focused)
                applyWidth.run();
        });
        ...
        btnTwo.setToggleGroup(dirGroup);
        btnOne.setToggleGroup(dirGroup);
        ...
        cbHwy.setTextFill(Color.web("#6bc5ff"));
        cbHwy.setStyle("-fx-font-size:11px;");
        ...
        cbEmerg.setTextFill(Color.web("#ff9944"));
        cbEmerg.setStyle("-fx-font-size:11px;");
        for (javafx.scene.Node n : new javafx.scene.Node[] { lblSpeed, tfSpeed, lblSpeedU, cbEmerg })
            n.setVisible(false);
        cbHwy.setOnAction(e -> {
        ...
        tfSpeed.setOnAction(e -> {
            try {
                view.setHighwayMinSpeed(Double.parseDouble(tfSpeed.getText().trim()));
            } catch (NumberFormatException ex) {
            }
    ...
    /**
     * Popup cấu hình chi tiết từng làn: loại xe + hướng rẽ riêng cho làn
     * xuôi/ngược.
     */
    private void showLaneConfigDialog(int laneCount, RoadSegment targetSeg) {
        ...
        for (int i = 0; i < laneCount; i++) {
            ...
            vehBox.setPrefWidth(260);
            vehBox.setAlignment(Pos.CENTER_LEFT);
            ...
            for (var def : vehDefs) {
                CheckBox cb = miniCb(def.icon(), vehPerLane[idx].contains(def.cat()),
                        c -> {
                            if (c)
                                vehPerLane[idx].add(def.cat());
                            else
                                vehPerLane[idx].remove(def.cat());
                        });
            ...
            movBox.setPrefWidth(250);
            movBox.setAlignment(Pos.CENTER_LEFT);
        ...
        scroll.setStyle(
                "-fx-background:#1a2030; -fx-background-color:#1a2030; -fx-border-color:#2a3550; -fx-border-width:1;");
    ...
    private void rebuildAllowedVehicles(CheckBox cbCar, CheckBox cbMoto, CheckBox cbBus,
        ...
        if (cbCar.isSelected())
            allowedVehicles.add(Lane.VehicleCategory.CAR);
        if (cbMoto.isSelected())
            allowedVehicles.add(Lane.VehicleCategory.MOTORBIKE);
        if (cbBus.isSelected())
            allowedVehicles.add(Lane.VehicleCategory.BUS);
        if (cbTruck.isSelected())
            allowedVehicles.add(Lane.VehicleCategory.TRUCK);
        if (cbBike.isSelected())
            allowedVehicles.add(Lane.VehicleCategory.BICYCLE);
    ...
    private HBox buildCtxIntersection(SimulationView view) {
        ...
        tfR.focusedProperty().addListener((ob, ov, focused) -> {
            if (!focused)
                tfR.getOnAction().handle(null);
        });
        ...
        spinBranch.getEditor().setStyle(
                "-fx-background-color:#2a3550; -fx-text-fill:#c8d0e8; -fx-font-size:12px; -fx-alignment:center;");
        spinBranch.valueProperty().addListener((ob, ov, nv) -> {
            if (nv != null)
                view.setRoundaboutBranches(nv);
        });
        lblBranch.setVisible(false);
        spinBranch.setVisible(false);
        ...
        HBox box = hbox(lblType, typeBox, vsep(), lblBranch, spinBranch, vsep(), lblR, sliderR, tfR, lblRUnit, vsep(),
                tip);
        box.setSpacing(8);
        return box;
    }
    ...
    private ToggleButton modeBtn(String icon, String label, String tooltip) {
        ...
        tb.selectedProperty().addListener((o, was, is) -> tb.setStyle(base + (is
     ...
     *
     * Bố cục:
     * [Loại đèn ▾] | [Đỏ: NN s] [Xanh: NN s] [Vàng: N s] | 🚦 Click vào giao lộ để
     * đặt đèn
     *
    ...
    private HBox buildCtxTrafficLight(SimulationView view) {
        ...
        typeBox.valueProperty().addListener((ob, ov, nv) -> {
            if (nv != null)
                view.setPendingLightType(typeKeys.getOrDefault(nv, "COUNTDOWN"));
        });
    ...
    private HBox buildCtxSimulation(SimulationView view, com.myteam.traffic.controller.VehicleSpawner spawner) {
        ...
        for (Slider s : new Slider[] { sCar, sMoto, sBike })
            s.setPrefWidth(55);

        sCar.valueProperty().addListener(
                (ob, ov, nv) -> spawner.setVehicleRatio(com.myteam.traffic.vehicle.VehicleType.CAR, nv.doubleValue()));
        sMoto.valueProperty().addListener((ob, ov, nv) -> spawner
                .setVehicleRatio(com.myteam.traffic.vehicle.VehicleType.MOTORBIKE, nv.doubleValue()));
        sBike.valueProperty().addListener((ob, ov, nv) -> spawner
                .setVehicleRatio(com.myteam.traffic.vehicle.VehicleType.BICYCLE, nv.doubleValue()));

        Label lblAgg = ctxLabel("Lái ẩu:");
        Slider sAgg = new Slider(0, 1, 0.3);
        sAgg.setPrefWidth(55);
        sAgg.valueProperty().addListener((ob, ov, nv) -> spawner.setAggressiveRatio(nv.doubleValue()));
    ...
    private void styleVehicleTypeCombo(ComboBox<com.myteam.traffic.vehicle.VehicleType> box) {
        box.setButtonCell(new javafx.scene.control.ListCell<>() {
            @Override
            protected void updateItem(com.myteam.traffic.vehicle.VehicleType item, boolean empty) {
                super.updateItem(item, empty);
